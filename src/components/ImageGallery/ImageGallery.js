  ...
  return (
    <div className="mb-8 md:mb-0 md:w-1/2">
      ...
      <div className="flex items-center justify-between gap-x-8 ">
        <div className="bg-red-300 w-full h-[64px] sm:h-[108px] flex items-center justify-center background rounded-lg cursor-pointer" onClick={()=>setMainImg(car.coverImage)}>
          <figure className="relative w-20 h-[26px] sm:w-[116px] sm:h-9" >
            <Image src={car.coverImage} alt="thumbnail1" fill />
          </figure>
        </div>
        <figure className="relative h-[64px] w-full sm:h-[108px] rounded-lg overflow-hidden cursor-pointer" onClick={()=>setMainImg(car.image1)}>
          <Image src={car.image1} alt="thumbnail2" fill />
        </figure>
        <figure className="relative h-[64px] w-full sm:h-[108px] rounded-lg overflow-hidden cursor-pointer" onClick={()=>setMainImg(car.image2)}>
