        ...
        <section className="md:flex md:justify-between md:gap-x-8 mb-8">
          ...
          <div className="bg-white p-4 md:p-6 rounded-[10px] md:w-1/2 self-stretch">
            ...
            <div className="grid grid-cols-4 gap-y-4 gap-x-6 md:gap-x-11 mb-8 md:mb-[68px] ">
              <span className="col-span-1 text-secondary-300 block text-xs md:text-xl font-medium">TypeCar</span>
              <span className="col-span-1 block text-xs md:text-xl font-semibold text-right">{car.cType.title}</span>
              <span className="col-span-1 text-secondary-300 block text-xs md:text-xl font-medium">Capacity</span>
              <span className="col-span-1 block text-xs md:text-xl font-semibold text-right">{car.capacity}Person</span>
              <span className="col-span-1 text-secondary-300 block text-xs md:text-xl font-medium">Steering</span>
              <span className="col-span-1 block text-xs md:text-xl font-semibold text-right">{car.steering}</span>
              <span className="col-span-1 text-secondary-300 block text-xs md:text-xl font-medium">Gasoline</span>
              <span className="col-span-1 block text-xs md:text-xl font-semibold text-right">{car.gasoline}L</span>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <Link href={`/cars/${car.hashId}/${car.slug}/payment`}>
                <Button
                  variant="contained"
                  className="bg-primary-500 w-[140px] h-14 hover:bg-primary-600 text-base font-bold"
                >
                  Rent Now
                </Button>
              </Link>
